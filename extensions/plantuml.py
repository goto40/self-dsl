...
class PlantUMLPreprocessor(markdown.preprocessors.Preprocessor):
    ...
    def _replace_block(self, text):
        ...
        # Parse configuration params
        img_format = m.group('format') if m.group('format') else self.config[
            'format']
        classes = m.group('classes') if m.group('classes') else self.config[
            'classes']
        ...
        return text[:m.start()] + etree.tostring(img).decode() + text[
                                                                 m.end():], True
...
class PlantUMLMarkdownExtension(markdown.Extension):
    ...
    def __init__(self, *args, **kwargs):
        self.config = {
            ...
            'format': ["png",
                       "Format of image to generate (png, svg or txt). Defaults to 'png'."],
